...
void setup() {
  ...
  for (int i = 0; i < CALIB_ITERS; i++){
    delayMicrosecondsFixed(CONTROL_PERIOD_US);
    mpu.getEvent(&a, &g, &temp);
...
void loop1() {
  Adafruit_Sensor* gyro = mpu.getGyroSensor();
  float ang = 0.0;

  Serial.println("Begin");
  last_time = micros();
  unsigned int tick = micros();
  ...
  while (true) {
    ...
    if (ang > 30/180*PI*0.38) break;
    runPeriodicallyMicros(10000);
  }

  unsigned int tock = micros();
...
void loop() {
  ...
  float angles[] = {0};
  int nang = sizeof(angles)/sizeof(angles[0]);

  ang = estimateAngle(ang);

  moveServo(angles[(int)(time/10)%nang]);
