...
void loop() {
  ...
  // Implementamos observador con sesgo
  
  float theta_obs1 = theta_obs + 0.02*omega_obs + 0.6899*(ang-theta_obs) + 0.0196*(ang_vel-omega_obs-bias);
  float omega_obs1 = -2.752*theta_obs + 0.572*omega_obs - 2.7636*(ang-theta_obs) + 0.2559*(ang_vel-omega_obs-bias) + 1.0728*ref;
  float bias1 = bias + 0.0307*(ang-theta_obs) + 0.0176*(ang_vel-omega_obs-bias);
  theta_obs = theta_obs1;
  omega_obs = omega_obs1;
  bias = bias1;

float datos[] = { ref, ang, theta_obs, ang_vel, omega_obs, bias};
  matlab_send(datos);
